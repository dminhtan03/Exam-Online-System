...
export default function StudentDashboard() {
  ...
  const checkAuthAndFetchExams = async () => {
    ...
    try {
      ...
      const testResponse = await fetch("http://localhost:9999/api/exams/test");
  ...
  const fetchExams = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || `Failed to fetch exams (${response.status})`
        );
      }
      ...
      const processedExams = data.data.map((exam) => ({
        ...
        status: exam.submissions?.[0]
          ? exam.submissions[0].verified
            ? "Verified"
            : "Submitted (Pending Verification)"
          : "Not Attempted",
        score: exam.submissions?.[0]?.verified
          ? exam.submissions[0].score
          : null,
      }));
      ...
      const completedExams = processedExams.filter(
        (exam) => exam.status === "Verified"
      ).length;
      const pendingExams = processedExams.filter(
        (exam) => exam.status === "Submitted (Pending Verification)"
      ).length;

      const verifiedExams = processedExams.filter(
        (exam) => exam.status === "Verified"
      );
      const averageScore =
        verifiedExams.length > 0
          ? Math.round(
              verifiedExams.reduce((sum, exam) => sum + (exam.score || 0), 0) /
                verifiedExams.length
            )
          : 0;
